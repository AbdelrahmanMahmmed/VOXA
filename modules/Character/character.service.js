...
const {
  ...
  UpdateCharacter,
  DeleteCharacter,
} = require("../../domains/Character/character.repo");
...
const {
  canCreateCharacter,
} = require("../../shared/middlewares/checklimitCharacter");

exports.createCharacterInDB = async (characterData, req, res) => {
  if (!canCreateCharacter(req.user)) {
    return res
      .status(403)
      .json({ message: "Character limit reached for this month." });
  }
  ...
  const specialist = getSpecialistMessage(
    characterData.Specialist,
    characterData.language,
  );
  const LanguageEnforcementMessage = getLanguageEnforcementMessage(
    characterData.language,
  );
