...
const { CreateFeedBackInDb, getFeedbackInDb, getAllFeedbackInDb, deleteOneInDb, UpdateIsResolved } = require('./feedback.service');
...
exports.UpdateIsResolved = async (req, res, next) => {
    try {
        const result = await UpdateIsResolved({isResolved : true} , req.params.feedbackId);
        res.json({
